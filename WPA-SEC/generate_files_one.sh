...
hcxpcaptool -o ${OUTDIR}o -O ${OUTDIR}O -z ${OUTDIR}z -j ${OUTDIR}j -J ${OUTDIR}J -E ${OUTDIR}E -I ${OUTDIR}I -U ${OUTDIR}U -P ${OUTDIR}P -T ${OUTDIR}T -X ${OUTDIR}X -g ${OUTDIR}g --netntlm-out=${OUTDIR}netntlm-out --md5-out=${OUTDIR}md5-out --md5-john-out=${OUTDIR}md5-john-out --tacacsplus-out=${OUTDIR}tacacsplus-out --eapol-out=${OUTDIR}eapol-out --network-out=${OUTDIR}network-out --hexdump-out=${OUTDIR}hexdump-out --hccap-out=${OUTDIR}hccap-out --hccap-raw-out=${OUTDIR}hccap-raw-out --nonce-error-corrections=128 -V "${INFILE[@]}" && if [ ${REMOVE} == true ]; then mkdir -p ${BACKUP_DIR}${time} ; for f in ${INFILE[@]} ; do if [ "${f:0:1}" == "/" ]; then f="${f:1:${#f}-1}" ; fi ; f_new="${f#*/}" ; fdir="${BACKUP_DIR}${time}${f_new}" ; mkdir -p ${fdir%/*} ; mv ${f} ${BACKUP_DIR}${time}${f_new} ; done ; fi 
